-- FUNCTION: aplus_unitofmesuregroup_get(integer, character varying, integer, integer, integer, timestamp without time zone, timestamp without time zone)

-- DROP FUNCTION aplus_unitofmesuregroup_get(integer, character varying, integer, integer, integer, timestamp without time zone, timestamp without time zone);

CREATE OR REPLACE FUNCTION aplus_unitofmesuregroup_get(
...
BEGIN
	...
	FROM unit_group x
	WHERE true
        AND (case when _code isnull or _code = 0 then true else ugp_entry = _code end)
        AND (case when _name isnull or _name Like '' then true else ugp_name::varchar Like '%' || _name || '%' end)
	...
	--Count record
	Select Max(row_num) into _sum_record from "#temp_doc";
