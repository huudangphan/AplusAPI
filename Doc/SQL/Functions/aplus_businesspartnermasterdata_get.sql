-- FUNCTION: aplus_businesspartnermasterdata_get(character varying, character varying, character varying, integer, integer, integer, integer, timestamp without time zone, timestamp without time zone)

-- DROP FUNCTION aplus_businesspartnermasterdata_get(character varying, character varying, character varying, integer, integer, integer, integer, timestamp without time zone, timestamp without time zone);

CREATE OR REPLACE FUNCTION aplus_businesspartnermasterdata_get(
	_code character varying DEFAULT ''::character varying,
	_name character varying DEFAULT ''::character varying,
	_type character varying DEFAULT ''::character varying,
	_branch integer DEFAULT 0,
...
BEGIN
	...
	Create Local Temp Table "#temp_doc" as 
		Select *, row_number() over (order by create_date desc) row_num 
		FROM partner 
		WHERE true
        	AND (case when _code = '' then true else card_code::varchar like '%' || _code || '%' end)
        	AND (case when _name = '' then true else card_name::varchar like '%' || _name || '%' end)
			AND (case when _type = '' then true else card_type::varchar like '%' || _type || '%' end)
			AND (case when _user_sign = 0 or _user_sign isnull then true else user_sign = _user_sign end)
        	AND (case when _branch = 0 or _branch isnull then true else dfl_branch = _branch end)
	...
	If (_page_index != 0 Or _page_size != 0) Then
